...
public class RegisterActivity extends AppCompatActivity {
    ...
    private EditText etDisplayName, etEmail, etPassword, etConfirm, etAddress;
    ...
    private void register() {
        ...
        if (email.isEmpty() || pass.isEmpty() || conf.isEmpty() || address.isEmpty()) {
            toast("Fill in all fields, including address");
            return;
        }
        if (!pass.equals(conf)) {
        ...
        auth.createUserWithEmailAndPassword(email, pass)
                .addOnSuccessListener(result -> {
                    ...
                    u.put("address", address);   // ✅ No need for if-condition
                    u.put("role", role);
                    ...
                    db.collection("users").document(uid).set(u, SetOptions.merge())
                            ...
                            .addOnFailureListener(e -> {
                                btnRegister.setEnabled(true);
                                toast("Save profile failed: " + e.getMessage());
                            });
                })
                .addOnFailureListener(e -> {
                    btnRegister.setEnabled(true);
